...
//#define SCPass
//MUST match the struct passed from the C# script!!! Otherwise, we'll read garbage data!
struct MeshProperties
{
    ...
    v2f vert(appdata_full i, uint instanceID : SV_InstanceID)
    {
        ...
        float4 pos = mul(_Properties[instanceID].TRSMatrix, i.vertex);
...
v2f vert(appdata_full i, uint instanceID : SV_InstanceID)
{
    ...
    o.uv2 = _Properties[instanceID].uvs;
    o.uv = i.texcoord;
    o.norm = UnityObjectToWorldNormal(i.normal);
    //Tangent stuff for... textures and lighting? I wish i understood this :(
    ...
    o.ambient = ShadeSHPerVertex(i.normal, 0); //float3(0, 0, 0); //ShadeSHPerVertex(o.norm, 0);
...
{
    // Sample textures
    fixed4 c = tex2D(_GroundTex, input.uv2);
    fixed4 c2 = tex2D(_MainTex, input.uv);
    ...
    //If we're the backface, flip the normal for correct lighting. 
    //Looks nicer when taking light from all angles but not as nice as not flipping and taking light from some angles...
    if (facing < 0.5)
    {
        normal *= -1.0;
    }
